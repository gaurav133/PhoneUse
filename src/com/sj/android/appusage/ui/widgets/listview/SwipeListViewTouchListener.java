...
public class SwipeListViewTouchListener implements View.OnTouchListener {
    ...
    private void generateAnimate(final View view, final boolean swap, final boolean swapRight, final int position,final float deltaX) {
        ...
        if (swipeCurrentAction == SwipeListView.SWIPE_ACTION_REVEAL && swap && swapRight) {
    ...
    public boolean onTouch(View view, MotionEvent motionEvent) {
        ...
        switch (MotionEventCompat.getActionMasked(motionEvent)) {
            ...
            case MotionEvent.ACTION_UP: {
                ...
                if (Math.abs(deltaX) > currentChild.getWidth() / 2 && (Math.abs(deltaY) < (currentChild.getHeight() * 3/4))) {
                    swap = true;
                    swapRight = deltaX > 0;
                }else{
                	swap = false;
